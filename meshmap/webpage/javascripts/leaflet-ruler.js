    ...
    _toggleMeasure: function() {
      ...
      if (this._choice){
        ...
        this._map.off('mousemove', this._moving, this);
		  this._map.removeControl(elevationPopup);
      }
    },
    _clicked: function(e) {
      ...
      if(this._clickCount > 0 && !e.latlng.equals(this._clickedPoints[this._clickedPoints.length - 2])){
        if (this._movingLatLong){
          polyline = L.polyline([this._clickedPoints[this._clickCount-1], this._movingLatLong], this.options.lineStyle).addTo(this._polylineLayer);
        }
        var text;
      ...
      this._clickCount++;
		var clickedPoints = this._clickedPoints.length;
		if (clickedPoints >= 2){
			...
			profile.run(function(error, result, response){
				...
				elevationPopup = L.control.elevation().addTo(map)
//        		var el = L.control.elevation().addTo(map)
				
				L.geoJson(geojson,{
					onEachFeature: elevationPopup.addData.bind(elevationPopup)
				}).addTo(map);
			});
			this._closePath();
		}
    ...
    _moving: function(e) {
	  ...
	  polyline.addLatLng(this._tempPoint);
